...
export default {
    ...
    methods: {
        ...
        onChangeTwspName(event) {
            var Value = event.target.value;
            this.student.town = Value;
        },
        
        getDetailStudent() {
            axios.get("/student/detail/" + this.studentId)
                .then((response) => {
                    this.student = response.data;
                    this.twspName = this.changeTwspName(this.student.region);
                    if (response.data.nrc_number) {
                        ...
                        this.student.nrc_number = nrcthird[1];
                    }
                    if (this.student.profile != null) {
                        this.image_profile = this.student.profile;
                    }
                    if (this.student.nrc_front != null) {
                        this.image_front = this.student.nrc_front;
                    }
                    if (this.student.nrc_back != null) {
                        this.image_back = this.student.nrc_back;
                    }
                })
                .catch((error) => {
        ...
        handleSubmit() {
            this.submitted = true;
            // this.student.nrc_valid = convertNumberService.convertMm2Eng(this.student.nrc_number);
            if (this.$v.$invalid) {
                return;
            }
            // this.student.nrc_valid = '';
            this.updateStudent();
        },
        ...
        updateStudent() {
            ...
            if (this.nrckey &&
                ...
                this.student.nrc_number) {
                nrc = convertNumberService.convertEng2Mm(this.nrckey)
                    .concat('/')
                    .concat(this.selectedNrcName)
                    .concat(this.selectedNrcNaing)
                    .concat(convertNumberService.convertEng2Mm(this.student.nrc_number));
            }

            const data = new FormData();
            data.append('name', this.student.name);
            data.append('school_name', this.student.school_name);
            data.append('gender', this.student.gender);
            data.append('date_of_birth', this.student.date_of_birth);
            data.append('nrc_number', nrc);
            data.append('phone_number_1', this.student.phone_number_1);
            data.append('phone_number_2', this.student.phone_number_2);
            data.append('email_1', this.student.email_1);
            data.append('email_2', this.student.email_2);
            data.append('region', this.student.region);
            data.append('address', this.student.address);
            data.append('town', this.student.town);
            data.append('profile', this.profile);
            data.append('nrc_front', this.nrc_front);
            data.append('nrc_back', this.nrc_back);
            axios.post("/student/update/" + this.studentId, data)
                .then(() => {
                    this.$router.push({
                        name: 'detailStudent',
